...
# --- CONFIGURATION ---
TARGET_SHEET = "IBPS_Bot_Data"  # Must match the file you created in Drive
RUNTIME_MINUTES = 5
DELAY_SECONDS = 60  # SAFETY BRAKE: Waits 60s to prevent 429 Errors

print(f"🚀 STARTING BEAST BOT v6.0 (Safety Mode)...")
...
try:
    ...
    print(f"✅ SUCCESS! Connected to your file: '{TARGET_SHEET}'")
    ...
    print(f"❌ ERROR: I cannot find '{TARGET_SHEET}'.")
    print(f"👉 Did you share it with: {creds_dict['client_email']} ?")
    exit(1)

# --- 3. AUTO-DETECT MODEL ---
def get_model():
    print("🔎 Selecting safe AI model...")
    try:
        models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
        # Force 1.5-flash (Highest Free Limits)
        if 'models/gemini-1.5-flash' in models: return 'models/gemini-1.5-flash'
        return models[0] if models else "models/gemini-pro"
    except: return "models/gemini-pro"

MODEL_NAME = get_model()
print(f"🤖 Engine Selected: {MODEL_NAME}")
...
while time.time() < end_time:
    ...
    try:
        ...
        if match:
            ...
            print(f"   ✅ SAVED to Google Sheet.")
            print(f"   💤 Sleeping for {DELAY_SECONDS}s to keep Google happy...")
        ...
        if "429" in str(e): 
            print(f"   🛑 Quota Limit Hit (429). Pausing for 2 minutes...")
            time.sleep(120)
        else:
